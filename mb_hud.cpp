...
void FullCustHudMessage(edict_t *pEntity, const char *msg_name, int channel, float pos_x, float pos_y, int gfx, Vector color1, Vector color2, int brightness, float fade_in, float fade_out, float duration)
{
	...
	WRITE_SHORT(FixedSigned16(pos_x, -(1 << 13)));	// x coord on the screen
	WRITE_SHORT(FixedSigned16(pos_y, (1 << 13)));	// y coord on the screen, changed it to positive value so that it aligns from top, else it messed the waypoint and path info messages
...
void StdHudMessage(edict_t *pEntity, const char *msg_name, int gfx, int time)
{
	...
	FullCustHudMessage(pEdict, msg_name, 2, 0.0f, 0.0f, gfx, color, color, 200, 0.03f, 0.1f, time);
}
...
{
	if (internals.GetHUDMessageTime() < gpGlobals->time)
		FullCustHudMessage(pEntity, msg_name, 3, 1.0f, 0.0f, gfx, color1, color2, 200, 0.03f, 1.0f, time);
}
...
void DisplayMsg(edict_t *pEntity, const char *msg)
{
	...
	// to prevent overloading HL engine, crashed if this message is used alot so just don't allow to print this message every frame
	...
	// the Y position is moved slightly down from the top of the screen so that the message doesn't block the icons for flags/objects DoD has at top left corner of the screen 
	FullCustHudMessage(pEntity, msg, 3, 0.0f, 0.1f, 0, color, color, 200, 0.03f, 0.03f, 5.0f);

}


/*
* displays a transparent HUD message at given position on the screen for given time
*/
void CustDisplayMsg(edict_t *pEntity, const char *msg, float x_pos, float y_pos, float time)
{
	...
	FullCustHudMessage(pEntity, msg, 3, x_pos, y_pos, 0, color, color, 200, 0.03f, 0.03f, time);
}
